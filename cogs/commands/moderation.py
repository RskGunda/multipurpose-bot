...
class Moderation(commands.Cog):
    ...
    async def hackban(self, ctx, userid, *, reason=None):
        ...
        try:
            ...
            await ctx.reply(embed=discord.Embed(
                description=
                f"<:anxTIck:1107932353654956043> | Successfully hackbanned {userid} for {reason}",
                color=0x2f3136))
        except:
            await ctx.reply(embed=discord.Embed(
    ...
    async def give(
        ...
        if guild.me.top_role >= ctx.author.top_role:
            embed = discord.Embed(
                color=0x2f3136,
                description=
                "<:anxAlert:1108101889687560336> | Your role is below the bot.")
            await ctx.send(embed=embed)
            return
        if member.top_role >= ctx.author.top_role:
            await ctx.send(
                "**<:anxAlert:1108101889687560336> | The provided user has roles that are above or have the same role as you.**"
            )
            return
        else:
...
Reason: `{entry.reason}`\n\n''')
    ...
    async def _softban(self,
        ...
        hacker = discord.Embed(
            color=0x2f3136,
            description=
            f"<:anxTIck:1107932353654956043> | Sucessfully soft-banned {member}.",
            timestamp=ctx.message.created_at)
    ...
    async def _kick(self,
        ...
        if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
            await ctx.send(
                "*<:anxAlert:1108101889687560336> | You cannot kick someone with a higher role than you!*"
            )
    ...
    async def _ban(self,
        ...
        if ctx.author.top_role.position > member.top_role.position or ctx.author == ctx.guild.owner:
            ...
            hacker = discord.Embed(
                color=0x00FFE4,
                description=
                f"<:anxTIck:1107932353654956043> | {member.display_name} has been successfully banned for the reason: `{reason}`",
                timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            hacker.set_thumbnail(url=f"{ctx.author.avatar}")
        ...
        if not ctx.author.top_role.position > member.top_role.position and ctx.author != ctx.guild.owner:
            embed = discord.Embed(
                color=0x00FFE4,
                description=
                f"*<:anxAlert:1108101889687560336> | You cannot ban someone with a higher role than you.*",
                timestamp=ctx.message.created_at)
            await ctx.send(embed=embed)
    ...
    async def _nuke(self,
        ...
        button1 = Button(label="No",
                         ...
                         emoji="<:anxNo:1107918844510605363>")
    ...
    async def _lock(self,
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="VortexBot | Lockdown",
    ...
    async def _unlock(self,
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="VortexBot | Unlockdown",
...
**Description:** {pack.description}
    ...
    async def temp(self, ctx, role: discord.Role, time, *,
        ...
        if role == ctx.author.top_role:
            embed = discord.Embed(
                description=
                f"<:anxAlert:1108101889687560336>> | {role} has the same position as your top role!",
                color=0x2f3136)
            ...
            if role.position >= ctx.guild.me.top_role.position:
                embed1 = discord.Embed(
                    description=
                    f"<:anxAlert:1108101889687560336>> | {role} is higher than my role, move my role above {role}.",
                    color=0x2f3136)
    ...
    async def delete(self, ctx, role: discord.Role):
        ...
        if role is None:
            embed2 = discord.Embed(
                description=
                f"<:anxAlert:1108101889687560336>> | No role named {role} found in this server .",
                color=0x2f3136)
