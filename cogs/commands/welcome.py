        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(thumbnail_link):
                ...
                hacker = discord.Embed(
                    color=0x2f3136,
                    description=
                    "<:anxTIck:1107932353654956043> | Successfully updated the welcome thumbnail url .",
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(image_link):
                ...
                hacker = discord.Embed(
                    color=0x2f3136,
                    description=
                    "<:anxTIck:1107932353654956043> | Successfully updated the welcome image url .",
                    timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 1:
                hacker = discord.Embed(
                    color=0x2f3136,
                    description=
                    f"<:anxCross:1107932430322647144> | You have reached maximum channel limit for channel which is one .",
                    timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
                ...
                if str(channel.id) in chh:
                    hacker1 = discord.Embed(
                        color=0x2f3136,
                        description=
                        f"<:anxCross:1107932430322647144> | This channel is already in the welcome channels list .",
                        timestamp=ctx.message.created_at)
                    hacker1.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=hacker1)
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 0:
                hacker = discord.Embed(
                    color=0x2f3136,
                    description=
                    f"<:anxCross:1107932430322647144> | This server dont have any welcome channel setupped yet .",
                    timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}",
                                  icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
                ...
                if str(channel.id) not in chh:
                    hacker1 = discord.Embed(
                        color=0x2f3136,
                        description=
                        f"<:anxCross:1107932430322647144> | This channel is not in the welcome channels list .",
                        timestamp=ctx.message.created_at)
                    hacker1.set_author(name=f"{ctx.author.name}",
                                       icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=hacker1)
    ...
    async def welctestt(self, ctx):
        ...
        if chan == []:
            hacker = discord.Embed(
                color=0x2f3136,
                description=
                f"<:anxCross:1107932430322647144> | Oops, Kindly setup your welcome channel first .",
                timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}",
                              icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
