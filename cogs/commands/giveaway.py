    ...
    async def dare(self, interaction: discord.Interaction, button):
        ...
        try:
            ...
            if len(str(interaction.channel.name)) <= 4:
                await interaction.response.send_message(f"<:anxCross:1107932430322647144> This Giveaway Has Been Ended.", ephemeral=True)'''
...
class give(commands.Cog):
    ...
    async def gstart(self, ctx):
        ...
        init = await ctx.send(embed=discord.Embed(
            title=f":tada:  Giveaway :tada: ",
            description=":tada:  **Answer the questions for the giveaway.**",
            color=self.color)
        ...
        questions= [
            ...
            "**[4]** **How many Winners you want for the Giveaway!** ? \n **Example: `1` | `2` | `3`**"
        ]
        ...
        for question in questions:
            embed = discord.Embed(
                ...
                color=0x030404
            ).set_footer(icon_url=self.bot.user.display_avatar.url, text=f"{NAME} | Giveaway")
            if index == 1:
            ...
            try:
                ...
                await ctx.send(embed=discord.Embed(
                    color=self.color,
                    description=f"<:anxTIck:1107932353654956043> | Please setup the giveaway again ):."
                ))
                return
            else:
        ...
        try:
            ...
            await ctx.send("<:anxCross:1107932430322647144> | Mention The Channel Correctly!, do it like {}.".format(ctx.channel.mention))
            return

        try:
            winners = abs(int(answers[3]))
            if winners == 0:
                await ctx.send(f"<:anxCross:1107932430322647144> | You did not enter the number correctly.")
                return
        except ValueError:
            await ctx.send(f"<:anxCross:1107932430322647144> | You did not enter an correctly.")
            return
        prize = answers[0].title()
        channel = self.bot.get_channel(channel_id)
        converted_time = convert(answers[2])
        if converted_time == -1:
            await ctx.send(f"<:anxCross:1107932430322647144> | Enter The Time Correctly (s|m|d|h)")
        ...
        giveaway_message = await channel.send(content=":tada:  **GIVEAWAY STARTED** :tada: ", embed=giveaway_embed, view=action_row)
    ...
    async def greroll(self, ctx, message: int):
      ...
      if str(message) not in data:
        ...
        try:
          ...
          await ctx.send(embed=discord.Embed(
                    color=self.color,
                    description=f"<:anxTIck:1107932353654956043> | You took too long to answer this question, Please setup the giveaway again ):."
                ))
          return
        try:
        ...
        with open(f"giveaway_users/{data[str(message)]['button_id']}.txt", "r") as file:
          ...
          result_embed.set_footer(icon_url=self.bot.user.display_avatar.url, text=f"{NAME} | Giveaway Ended")
